...
class Login extends Component {
  ...
  submitBtnHandler = async (ev) => {
    ...
    if((obj.email === "" || obj.password === "" || this.state.errorEmail || this.state.errorPassword)){
      ...
      try{
        const res = await login(this.state.email,this.state.password,ev)
      ...
      // finally{
      //   if(!this.state.error) {this.props.history.push('/addData')} 
      // }
    }
  }
    ...
    return (
      <main className={classes.main}>
        ...
        <Paper className={classes.paper}>
          ...
          <br/>
          <p style = {{fontStyle : "italic"}}>Don't have an Account? <Link to='/signup'>Signup</Link></p>
        </Paper>
      </main>
    );
  }
